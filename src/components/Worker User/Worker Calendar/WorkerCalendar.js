...
const WorkerCalendar = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/worker/orders/${workerId}`);
  ...
  const handleStatusChange = async (index, newStatus) => {
    try {
      ...
      await axios.put(`http://localhost:3001/worker/executed-status-change/${events[index]._id}`, updatedEvent);
